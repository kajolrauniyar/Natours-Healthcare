...
/*Contact*/
function addContact($data, $is_die =false){
	global $conn;
...
function deleteContact($id, $is_die = false){
	global $conn;
	$sql = "DELETE FROM contact WHERE id = ".$id;//delete the data which id is matched
	if($is_die){
		debugger($sql, true);
	}
...
function updateContact($data, $con_id, $is_die = false){
	global $conn;
	...
	$query = mysqli_query($conn, $sql);
	if($query){
		return $con_id;//true likheda pani hunxa ... incase kahile hamilai Doctor check garnu parxa tesle doc_id pass gardiyeko
	} else {
		return false;
	}

}
function getAllContact($is_die=false){
	global $conn;
...
function getContactById($id, $is_die=false){
	global $conn;
